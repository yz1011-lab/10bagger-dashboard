...
QUANT_AUDITOR_WEBHOOK = f"{WEBHOOK_BASE}/quant-auditor"
...
def trigger_quant_auditor(tickers=None):
    """Trigger quant-auditor webhook to crawl FMP data and update Bayesian probabilities."""
    try:
        payload = {
...
with tab2:
    ...
    with trig_c1:
        st.markdown("手動觸發 **Quant Auditor** 爬蟲，抓取最新 FMP 財務數據並以貝葉斯模型更新 P(H) 機率。")
    with trig_c2:
        if st.button("🚀 立即更新機率", type="primary", use_container_width=True):
            with st.spinner("正在呼叫 Quant Auditor webhook..."):
                qa_status, qa_resp = trigger_quant_auditor()
            if qa_status == 200:
                st.success(f"Quant Auditor 已觸發成功！(HTTP {qa_status})")
                st.caption(f"回應: {qa_resp[:200]}")
                st.info("請等待 1-2 分鐘後重新整理頁面查看更新後的機率。")
            else:
                st.error(f"觸發失敗 (HTTP {qa_status}): {qa_resp[:200]}")
    st.markdown("---")
